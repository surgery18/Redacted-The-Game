...
export const DocumentView: React.FC<DocumentViewProps> = ({ 
  ...
  return (
    ...
    >
      {/* OS Window Frame */}
      <div className="relative w-[850px] min-h-[1100px] bg-stone-200 border border-stone-400 shadow-[0_20px_50px_rgba(0,0,0,0.5)] rounded-sm flex flex-col overflow-hidden">
        
        {/* Window Title Bar */}
        <div className="bg-stone-300 border-b border-stone-400 h-8 flex items-center justify-between px-2 select-none">
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-tech text-stone-600 uppercase tracking-widest">DOC_VIEWER_V1.4.EXE</span>
            <span className="text-[10px] font-bold text-stone-800">:: {data.title}</span>
          </div>
          <div className="flex gap-1">
             <div className="w-3 h-3 bg-stone-400 rounded-sm flex items-center justify-center"><Minus size={8} /></div>
             <div className="w-3 h-3 bg-stone-400 rounded-sm flex items-center justify-center"><Square size={8} /></div>
             <div className="w-3 h-3 bg-red-400 rounded-sm flex items-center justify-center"><X size={8} color="white"/></div>
          </div>
        </div>

        {/* Scanned Document Canvas */}
        <div 
           className="flex-1 scanned-texture p-16 text-black font-typewriter text-lg leading-relaxed relative"
           onClick={(e) => {
